...
export function AuthForm() {
  ...
  const handleGoogleCallback = async (code: string) => {
    try {
      ...
      if (response.data.success) {
        ...
        const { _id, fullName, email, profilePicture, role } =
          response.data.user;
        const userData = { id: _id, fullName, email, profilePicture, role };

        registerUser(userData);

        toast({
          title: "Success",
          description: "You've been logged in successfully via Google.",
        });
  ...
  const handleGitHubCallback = async (code: string) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast({
          title: "Success",
          description: "You've been logged in successfully via GitHub.",
        });
  ...
  useEffect(() => {
    ...
    if (code) {
      if (searchParams.get("scope")?.includes("google")) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Extract only the required details for the user interface
      const { _id, fullName, email, profilePicture, role } = response.data.user;
      const userData = { id: _id, fullName, email, profilePicture, role };
